...
```{r, solution_01a}
df %>%
  gather(key = "x", value = "points", 1:2) %>% 
  ggplot(mapping = aes(x = points, y = y)) + 
  geom_point() + 
...
Based on the visualization it's hard to see the relationship between the input `x1` and the response but for `x2` we can clearly see as the values of `x2` increases to zero from a negative value the value of response increases and as the values of `x2` increases from zero the value of the response decreases.  
...
Since the value of `x1` is always `positive` irrespective of response variable it is statistically significant.  
...
There are `4` features present in the model.  
1 - Intercept  
2 - x1  
3 - x2  
4 - x1:x2 `interaction` between inputs.  

All features are statistically significant except the intercept.  
...
There are `8` features present in the model plus the intercept.  
Only the intercept and $x_2^2$ features are statistically significant.  
...
None of the features look statistically significant except the intercept.  
...
No none of the features is statistically significant.  
...
```{r}
train <- purrr::reduce(list(train01, train02, train03, train04, train05, train06), dplyr::full_join)
```
```{r}
train 
```
...
max(train$r.squared)
```
`Model-6` is best according to r-squared.  

```{r}
min(train$AIC)
```
`Model-3` is best according to AIC.    

```{r}
min(train$BIC)
```
`Model-3` is best according to BIC.    
...
What do you think?  
Yes the prediction is consistent with the model performance ranking based on BIC.  
Model-6 is considered to be the worst by BIC.  
Relationship between size of confidence interval to the size of prediction interval is the defining characteristic. The total uncertainty is dominated by the uncertainty on average.   This is because all coefficients are very very uncertain.  
...
```
Since the value of the ratio is greater than one so the numerator is bigger than denominator and we took model-3 in the numerator.  
Therefore `model-3` is better than model-6.  
...
What do you think?  
The value of the coefficient posterior distribution is very small in case of very strong compared to strong case.  
The uncertainty on each parameter also decreases for very strong case compared to strong case.  
But as we can see in the visualization in case of model-3 there are no statistically significant feature in very strong whereas $x_2^2$ was significant in strong.  
Similarly in case of `model-6` $x_2^4$ is statistically significant for very strong whereas none of the features were significant for just strong.  
...
As we can see for `strong` case `model-3` is better whereas for `very-strong` case `model-6` is better.  
...
```{r}
post_pred_summary_viz_03_weak <- summarize_lm_pred_from_laplace(mvn_result = laplace_03_weak, Xtest = X03_test, num_samples = 5000)

post_pred_summary_viz_03_strong <- summarize_lm_pred_from_laplace(mvn_result = laplace_03_strong, Xtest = X03_test, num_samples = 5000)

post_pred_summary_viz_03_very_strong <- summarize_lm_pred_from_laplace(mvn_result = laplace_03_very_strong, Xtest = X03_test, num_samples = 5000)
```
...
```{r}
post_pred_summary_viz_06_weak <- summarize_lm_pred_from_laplace(mvn_result = laplace_06_weak, Xtest = X06_test, num_samples = 5000)

post_pred_summary_viz_06_strong <- summarize_lm_pred_from_laplace(mvn_result = laplace_06_strong, Xtest = X06_test, num_samples = 5000)

post_pred_summary_viz_06_very_strong <- summarize_lm_pred_from_laplace(mvn_result = laplace_06_very_strong, Xtest = X06_test, num_samples = 5000)
```
...
What do you think?  
As the prior standard deviation is decreased the model becomes very uncertain.  
We can see that the confidence interval ranges from `4000` in case of weak.  
Yes for both `model-3` and `model-6` the posterior predictions are consistent with the posterior coefficients.   
...
```
For both the models only the intercept and $x_2^2$ have not been turned off.  
The final model tuned have kept the same features turned on but the values of coefficients are different.  