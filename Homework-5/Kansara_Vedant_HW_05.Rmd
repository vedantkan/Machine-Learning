...
From the above two equations we can calculate $\mu_0$ as
```{r}
mu_0 = (135 + 115) / 2
tau_0 = (135 - mu_0) / 2
```
...
```{r}
percentile_25 <- qnorm(0.25, mean = mu_0, sd = tau_0)
percentile_75 <- qnorm(0.75, mean = mu_0, sd = tau_0)
percentile_25
percentile_75
```

The interval between these two quantiles contains `50%` of the probability mass.  
We therefore have a 50% chance to find values between `121.6276` and `128.3724`.  
...
$$
$$
\frac{N}{\sigma^2} = \frac{1}{\tau_N^2} - \frac{1}{\tau_0^2}
$$
$$
N = \sigma^2 \cdot \left(\frac{1}{\tau_N^2} - \frac{1}{\tau_0^2} \right)
$$
```{r}
sig <- 5 
tau_n1 <- 2
...
```{r}
N1 = sig^2 * ((1/tau_n1^2) - (1/tau_0^2))
N1

N2 = sig^2 * ((1/tau_n2^2) - (1/tau_0^2))
N2
```
When $\tau_n =2$ the number of observations required are `6`.  
When $\tau = 0.5$ the number of observations required are `99`.  
...
$$
$$
N = \sigma^2 \cdot \left(\frac{1}{\tau_N^2} - \frac{1}{\infty} \right)
$$
$$
N = \sigma^2 \cdot \left(\frac{1}{\tau_N^2} - 0 \right)
$$
$$
...
$$
p\left(\mu \mid \mathbf{x}, \sigma\right) \propto \prod_{n=1}^{N} \left( \frac{1}{\sigma \sqrt{2\pi}} \exp \left[\frac{-1}{2\sigma^2} \left( x_n  - \mu \right)^2 \right] \right) \cdot \frac{1}{\tau_0 \sqrt{2\pi}} \exp \left[\frac{-1}{2\tau_0^2} \left(\mu - \mu_0 \right)^2 \right]
$$
...
The expression is same as that of the posterior mean provided in lecture. This is because since the distribution of the posterior will be a Gaussian and we know that for a Gaussian distribution `Mean = Mode = Median`. 
...
$$
$$
\log \left(p \left(\sigma \mid \mathbf{x}, \mu\right) \right) \propto - N\log \left(\sigma\right) -  \frac{Nv}{2\sigma^2}   -\lambda\sigma
$$

The `sufficient` represents that the `information content` of the observations can be represented by the sample mean. Since we have `$\mu$` we can calculate the sample mean by subtracting `$\mu$` from each observation. Hence `v` is being referred to as `sufficient`.   
No after making use of `v` the un-normalized log-posterior does not depend in individual observation.  
...
{
  -1 * ((N*log(sigma)) + ((v*N) / (2*sigma^2)) + (lambda * sigma))
}
```
...
What do you think?  
The log-posterior has `one` mode.  
Yes the shape of the log-posterior does look like a parabola.  
...
$$
The log-derivative of the inverse link function is
$$
...
What do you think?  
Yes the log-posterior appears more parabolic now.  
...
opt_sine_01
```
The `fnscale` argument inside control tells `optim()` to maximize instead of minimize.  
...
`No` the posterior mode remains the same because we have a gaussian. Only the number of iterations the optim required to find the maximum changed.  
