...
postmeans_quad_small_noprior <- Xmat_quad_small %*% solve(t(Xmat_quad_small) %*%  Xmat_quad_small) %*% t(Xmat_quad_small) %*% y_quad_small_col
...
postmeans_quad_big_noprior <- Xmat_quad_big %*% solve(t(Xmat_quad_big) %*%  Xmat_quad_big) %*% t(Xmat_quad_big) %*% y_quad_big_col
...
```{r}
quad_feature_small <- solve(t(Xmat_quad_small) %*%  Xmat_quad_small) %*% t(Xmat_quad_small) %*% y_quad_small_col

quad_feature_small - 1
quad_feature_small + 1
```


```{r}
quad_feature_big <- solve(t(Xmat_quad_big) %*%  Xmat_quad_big) %*% t(Xmat_quad_big) %*% y_quad_big_col
...
What do you think?  
Assuming $\sigma = 1$ We can see that for both cases, small and large data case the quadratic feature is statistically significant.  
...
No the initial guess does not impact the result because it is a linear model and there will be onlt one unique maximum.  
