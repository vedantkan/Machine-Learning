...
dim(Xmat_quad_small)
```
Therefore the big data set has `30` rows and `3` columns and the small data set has `5` rows and `3` columns.  

```{r}
Xmat_quad_small
```
All the values in the first column are `1`.  
This is because we introduced a fake input term $x_{n,0}$ which corresponds to intercept term is just equal to `1`.  
...
```
Each posterior covariance matrix is a matrix of `3` rows and `3` columns.  
...
We know that the posterior standard deviation are square root of the main diagonal of the covariance matrix. 

So for small data case  
...
No the values would remain the same as in the case of no prior the value of MAP of $\beta$ does not depend on $\sigma$.  
...
What do you think?  
Assuming $\sigma = 1$ We can see that for large data case, the quadratic feature is not statistically significant.  
whereas for small data case it is.  
...
```
In case big data both `inform` and `no-prior` have same standard deviation.  
But in case of small data the values of standard deviation for `inform` case are smaller than compared to `no-prior` case.  
...
What do you think?  
In the case of small data the standard deviation of the regression coefficients is less when using informative prior compared to the vague prior.  
In case of big data is it almost silimar in both cases whether using the informative prior or the vague prior.  
...
```{r, solution_03b_b}
lm_logpost(as.vector(c(1,1,1,1)), info_quad)
```
...
No the initial guess does not impact the result because it is a linear model and there will be only one unique maximum.  
...
```
The probability of $\beta_2$ being greater than zero is `0`.  
...
The rows correspond to number of prediction or test points.  
The columns correspond to random posterior samples of the $\beta$ parameter.  
...
```
The length of the `error_quad_post_01` is `30`.  
...
We are averaging over the number of test points (length of the error vector) in this which is `30`.   
No the value of MAE is different in each case because each case we have predicted the outcome with particuluar values of $\beta$ for that case.  
So case-1 have a particular values of $\beta$ and will predict the outcome with those values. Whereas case-2 will have different value of $\beta$ and will predict the outcome with that set of values.  
Therefore the predicted outcome will be different in each case and hence we will have different errors in each case, so the MAE are different.  
...
Which function?  
As each columns represents the random sample of the $\beta$ parameter we should use the `colMeans()` function to get the average value associated with each column.  
...
```
```{r}
as.character(MAE_quad[3])
as.character(mae_quad_post_03)
```
Both values are same but still getting FALSE in the result.  
...
The uncertainty in the $\beta$ induces the uncertainty in the mean trend.  
...
There are `150` rows in the `design matrix` and `50` rows in the `test matrix`.  
...
The models with degrees of freedom higher than `10` seems to overfit the data as we can see the training data MAE is very low and the test data MAE is increasing.  
...
According to the above figure `model-7` is the best but `model-9` is also pretty good.  
...
What do you think?  
The assessment from the single train/test split shows that the models with higher degree of freedom preforms best on the training set as we can see model-49 is the best. We need re-sampling scheme to tell which model does not overfit the data.  
The evidence based model calculates the log-evidence which penalizes the performance based on the log-determinant of the Hessian. The log-determinant of the Hessian scales as the number of terms.  Hence it gives us the best model.  
