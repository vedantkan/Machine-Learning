...
Each input appears to have roughly the same scale.  
...
Out if the first figure `x03` seems to be positively related with the output as we can see the value of output increases as the value of x03 increases.  

In the second figure `x50` looks somewhat positively related.  

In the third figure none of the inputs look related.  
...
modA %>%
  ...
  filter(p.value < 0.05)
```
We can see that only the intercept and other 6 features are important which can be conformed by our visualiation above.  
...
loss_ridge(beta_1, check_info_A)
```
...
loss_ridge(beta_2, check_info_A)
```
...
The ridge regularized coefficients are very similar to the MLE estimate.  
...
The ridge regression coefficients are very different from the MLE estimates.  
...
lambda_grid <- log_seq(0.001, 1000, 101)
```
...
What do you think?  
All the coefficients values decreases and is set to near zero as the strength of regularization increases.   
...
**To check if you programmed `loss_lasso()` correctly**, test the function with a guess of -1.2 for all regression coefficients. If you programmed `loss_lasso()` correctly you should get a value of 181.1651. As another test, use 0.2 for all regression coefficients. If you programmed `loss_lasso()` correctly you should get a value of 22.88061.  
...
```{r}
loss_lasso(beta_1, check_info_A)
```

Test `loss_lasso()` with 0.2 for all regression coefficients.  

```{r}
loss_lasso(beta_2, check_info_A)
```
...
```{r}
check_lasso_fit_lowpenalty <- fit_regularized_regression(lambda_use = 0.001, loss_func = loss_lasso, my_info =  reg_info)
```
...
The regularized lasso estimate are very similar to the MLE estimates.  
...
```
The regularized lasso estimates are very different from the MLE estimates.  
...
What do you think?  
Compared to the ridge figure in lasso figure we see that there are no estimates for the coefficient after `log(lambda) == 0`.    
This is because unlike ridge, lasso turns off the estimate to zero for the coefficients which lasso thinks are not important unlike ridge which do not completely set the estimate to zero.  
...
```{r}
x_inter <- train_test_lasso_results %>% filter(MSE_test == min(MSE_test)) %>% select(lambda)
```
...
train_test_lasso_results %>%
  ...
  geom_line() + 
  geom_hline(mapping = aes(yintercept = min(MSE_test)), color = "red", linetype = "dashed") +
  geom_vline(xintercept = log(x_inter$lambda), color = "red", linetype = "dashed")
```
...
As we can see the when we use `lambda == 0.25118866` we get the least MSE on the test set.  
...
```{r}
best_estimate <- lasso_path_results %>%
  filter(between(lambda, 0.251, 0.252))
```

```{r}
viz_compare_to_mles(best_estimate, modA)
```
...
How do they compare?  
Both the figures are very similar.  
In this figure we only see the values of lambda until all the coefficients estimate are turned off where we checked for extra values of lambda.  
...
lasso_cv <- cv.glmnet(Xenet, yenet, aplha = 1, nfolds = 5, lambda = lambda_grid)
```
...
plot(lasso_cv)
```
The best lasso model recommends 6 feature but the one standard error rules recommends using the model with 5 features.  
...
coef(lasso_cv)
```
Based on the results of tuned lasso model it recommends unsing x3, x6, x16, x21, and x33 features.  
...
lfB %>%
  ggplot(mapping = aes(x = value, y = y)) + 
  geom_point(alpha = 0.33, size = 1.2) +
  facet_wrap(~name)
```
...
```{r}
corrplot::corrplot(coor)
```

```{r}
corrplot::corrplot(coor, type = 'upper', method = 'number')
```
Based on the visualization we can say that the inputs are very correlated.  
...
XBpairs %>% 
  cor() %>%
  corrplot::corrplot(method = 'square', type = 'upper', order = 'hclust', hclust.method = 'ward.D2')
```
...
enet_default <- train(y ~ (.)^2, data = dfB, method = "glmnet", metric = my_metric, trControl = my_ctrl, preProcess = c("center", "scale"))
```
...
The parameter conditions with `alpha ==1` and `lambda == 0.06870713` are considered to be the best.  
Since `alpha==1` it is considered to be `lasso`.  
Yes the feature correlation does seem to impact the model behavior because ridge does not work when features are highly correlated and we can see that as the elastic net has completely tuned the penalty parameter to lasso.  
...
Add your code chunks here.  
```{r}
enet_default$finalModel %>%
  varImp()
```
Yes the rankings are consistent with the magnitude of the coefficients in last problem.  
...
dfC %>%
  ggplot(mapping = aes(x = x, y =y)) + 
  geom_point(mapping = aes(color = v)) + 
  geom_smooth(mapping = aes(color = v), formula = y ~ x, method = lm)
```

Yes, the output and input relationship does appear to vary across the levels of `v`.  
As we can see from the above figure that for levels `A` and `B` the response `y` increases as the value of input `x` increases.  
And for `D` it decreases but not in great amount, but for level `C` the value of response decreases drastically as the value of `x` increases.  
...
5 coefficients are estimated.  
THe intercept correspond to `VA`.  
...
```{r}
broom::glance(modC_add)
```

```{r}
broom::glance(modC_int)
```

On the training set based on AIC and BIC the interactive model seems to be better.  
