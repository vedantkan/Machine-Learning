...
What do you think?  
`a` is a priori number of "events" and `b` is a priori number of "non-events".  
So this specification correspond to `1` prior trial or prior target.  
It is uninformed because it is just 1 prior trial. We are usnusre about those particular values because ith just 1 trials we cannot be certain.  
### 1b)
...
What do you think?  
Since we are using a conjugate prior the posterior distribution will have same functional form as the prior. So the posterior will have a `Beta` distribution.  
...
summary_post_df_focus_from_vague <- post_df_focus_from_vague %>% mutate(post_avg = anew / (anew + bnew),
...
summary_post_df_focus_from_vague %>% 
  ...
  geom_point(mapping = aes(y = num_events / (num_events + num_trials)), color = "red")
```

What do you think?  
Yes there are many players with MLE outside the posterior uncertainty level.  
The posterior mean for players with player ID `169` and `300` seems quite close to the MLE.  
...
```{r, solution_01g}
summary_post_df_focus_from_vague %>% 
  ggplot(mapping = aes(as.factor(num_trials))) + 
  ...
  geom_point(mapping = aes(y = (num_events / (num_events + num_trials)), group = player_id), position = position_dodge(0.2), color = "red")
```

What do you think?  
The players with the least number of trials that is `1` trial and `3` trials have high posterior uncertainty on the event probability.  
...
$$
p \left( \mu \mid \left( \left(m, N\right)_j \right)_{j=1}^{J} \right) \propto \prod_{j=1}^{J} \left(\mu^{m_j} \left(1 - \mu  \right)^{N_j - m_j} \right) \times \left(\mu^{a-1} \left(1 - \mu^{b-1} \right) \right)
$$

$$
\log \left(p \left( \mu \mid \left( \left(m, N\right)_j \right)_{j=1}^{J} \right) \right) \propto \log \left(\prod_{j=1}^{J} \left(\mu^{m_j} \left(1 - \mu  \right)^{N_j - m_j} \right) \times \left(\mu^{a-1} \left(1 - \mu^{b-1} \right) \right) \right)
$$

$$
\log \left(p \left( \mu \mid \left( \left(m, N\right)_j \right)_{j=1}^{J} \right) \right) \propto \log \left[ \prod_{j=1}^{J} \left(\mu^{m_j} \left(1 - \mu  \right)^{N_j - m_j} \right) \right] + \log \left[\left(\mu^{a-1} \left(1 - \mu^{b-1} \right) \right) \right]
$$

$$
\log \left(p \left( \mu \mid \left( \left(m, N\right)_j \right)_{j=1}^{J} \right) \right) \propto \sum_{j=1}^{J} \log \left[  \left(\mu^{m_j} \left(1 - \mu  \right)^{N_j - m_j} \right) \right] + \log \left[\left(\mu^{a-1} \left(1 - \mu\right)^{b-1}  \right) \right]
$$

$$
\log \left(p \left( \mu \mid \left( \left(m, N\right)_j \right)_{j=1}^{J} \right) \right) \propto \sum_{j=1}^{J}  \left[ \log \left(\mu^{m_j} \right) + \log \left( \left(1 - \mu  \right)^{N_j - m_j} \right) \right] + \log \left(\mu^{a-1} \right) + \log \left( \left(1 - \mu\right)^{b-1}  \right) 
$$

$$
\log \left(p \left( \mu \mid \left( \left(m, N\right)_j \right)_{j=1}^{J} \right) \right) \propto \sum_{j=1}^{J} \left[m_j \cdot \log \left(\mu \right) + \left(N_j - m_j \right) \cdot \log \left(1 -\mu \right)  \right] + \left(a-1 \right) \log \left(\mu \right) +\left(b-1 \right) \log \left(1 - \mu \right)
$$

$$
\log \left(p \left( \mu \mid \left( \left(m, N\right)_j \right)_{j=1}^{J} \right) \right) \propto \log \left(\mu \right)\sum_{j=1}^{J} m_j  + \log \left(1 -\mu \right) \sum_{j=1}^{J} \left(N_j - m_j \right)   + \left(a-1 \right) \log \left(\mu \right) +\left(b-1 \right) \log \left(1 - \mu \right)
$$

$$
\log \left(p \left( \mu \mid \left( \left(m, N\right)_j \right)_{j=1}^{J} \right) \right) \propto \log \left(\mu \right) \left[\sum_{j=1}^{J} m_j + \left(a-1 \right) \right] + \log \left(1 -\mu \right) \left[\sum_{j=1}^{J} \left(N_j - m_j \right) + \left(b-1 \right) \right]
$$
...
$$
\log \left(p \left( \mu \mid \left( \left(m, N\right)_j \right)_{j=1}^{J} \right) \right) \propto  \left[\bar{m}J + \left(a-1 \right) \right] \log \left(\mu \right) +  \left[J\left(\bar{N} - \bar{m} \right) + \left(b-1 \right) \right] \log \left(1 -\mu \right)
$$
...
What do you think?  
The unknown "pooled" estimate have a `Beta` posterior distribution.  
...
summary_post_df_focus_pooled <- post_df_focus_pooled %>% mutate(post_avg = anew / (anew + bnew),
                                                                post_q05 = qbeta(0.05, shape1 = anew, shape2 = bnew),
                                                                post_q95 = qbeta(0.95, shape1 = anew, shape2 = bnew))
```
...
What do you think?  
Yes all players MLEs are outside the posterior uncertainty interval.  
No, there are no players with posterior mean values that are quite close to the MLEs.    
...
What do you think?  
The "pooled" estimate seems incorrect for this application because the number of events and number of trials are very different for each player.  
So some players we only have couple of trials and for some we have hundreds and we are assume the priors to be the avg. of all players irrespective of number of observations.  
The data is not very balanced and hence the visualization doesn't "feel right".  
...
From the above figure we can say that players with greater than 25 targets we can see some sort of a distribution unlike players who have less than 25 targets.  
...
df_24 %>% summary()
```
From the above table we can see that the minimum value of the num_trials data is 25.  
Hence our filtering operation is correct.  
...
We have new a = 15.8 and new b = 7.83.
So the a priori trials are 15.8 + 7.83 = `23.63`.  
...
```{r, solution_03h}
qbeta(0.05, shape1 = ab_emp_bayes[1], shape2 = ab_emp_bayes[2])
qbeta(0.95, shape1 = ab_emp_bayes[1], shape2 = ab_emp_bayes[2])
```
...
For players with higher number of trials have MLE very close to the posterior mean.  
And there are several observation for which the players MLE is closer to the posterior uncertainty interval.  
...
summary_post_df_focus_empbayes %>%
  ...
  geom_linerange(mapping = aes(ymin = post_q05, ymax = post_q95, group = player_id), position = position_dodge(0.2))+
  geom_point(mapping = aes(y = post_avg, group = player_id), position = position_dodge(0.2)) + 
  geom_point(mapping = aes(y = num_events / num_trials, group = player_id), position = position_dodge(0.2), color = "red")
```
...
What do you think?  
Even with the informative prior for very small number of trials such as `1,2,3` the MLE is not closer to posterior mean. But compared to uninformative prior the informative trials give much better results as the sample size increase. Even with a little increase in sample size around `10-15` it gives us much better estimates  compared to uninformative as we can see MLE is much closer to the 90% C.I around the posterior mean and for very large sample size that is `>50` it gives us very accurate estimates.  
...
```{r, solution_05a}
summary_post_df_all_empbayes <- summary_post_df_all_empbayes %>% mutate(prob_grt_67 = 1 - pbeta(0.67, anew, bnew))
```
...
```{r, solution_05b}
summary_post_df_all_empbayes %>% arrange(desc(prob_grt_67))
```
Beside the `prob_grt_67` the all players have very high number of trials or number of targets.    
...
```{r, solution_05c}
summary_post_df_all_empbayes %>% arrange(prob_grt_67)
```
The smallest number of trials is associated with the player having id `506` having just `47` trials.     
...
```{r, solution_05d}
summary_post_df_all_empbayes %>% arrange(num_trials) %>% filter(prob_grt_67 > 0.75)
```
...
What do you think?  
When we used the completely pooled data we got a posterior average of around `0.67` which means the overall posterior catch rate when we take each and every player into consideration irrespective of the number of trials or the liklihood catch rate is `0.67` which indicates that if a player has a catch rate greater than `0.67` he can be considered a good player.   
...
summary_post_df_all_empbayes %>%
  ...
  geom_point(mapping = aes(color = num_trials), size = 3) +
  theme_bw()
...
What do you think?  
From the above figure we should take risk on the players which have high post_avg values but less (post_q95-post_q05) value and less number of trials.  
Because lesser the value of (post_q95-post_q05) more confident we are on the post_avg of the player.  

```{r}
summary_post_df_all_empbayes %>% filter(num_trials < 100) %>% arrange(desc(post_avg))
```

For example we can see one dot one black dot on the right side which has very high post_avg and good value of (post_q95-post_q05). 

From the above table we can see its player `831` who in `23`trials have post_avg = `0.83` and (post_q95-post_q05) = `~0.177`.

We can also consider the player `418` which has relatively slight less post_avg compared to player `831` but the value of (post_q95-post_q05) = `~0.135` which is just not lesser than player `831` but also over large sample size than player `831`.  So we can be more confident in player `418`.  
