...
Yes the data set is almost perfectly balanced.  
...
```
Yes the upper and lower bounds on `pred_prob` are in fact between 0 and 1.  
...
What do you think?  
Yes the observed binary response do follow the model predicted probability.  
We can see initially the binary response is `1` and the predicted probability is also near that.  
As the value of x approaches to `zero` the predicted probability curve is bent down to zero.      
...
```{r, solution_02b}
correct <- model_class_0.5 %>% count(obs_class == pred_class)
acc <- correct[2,2] / 125
acc
```
...
your response here.  
`6` events are associated with `false-positive`.  
`53` events are associated with `true-negetive`.  
...
```{r, solution_02d}
confu_mat_0.5 <- model_class_0.5 %>% count(pred_class, obs_class)
```

```{r}
sensitivity_0.5 <- (confu_mat_0.5$n[confu_mat_0.5$pred_class=="event" & confu_mat_0.5$obs_class=="event"]) / (confu_mat_0.5$n[confu_mat_0.5$pred_class=="event" & confu_mat_0.5$obs_class=="event"] + confu_mat_0.5$n[confu_mat_0.5$pred_class=="non_event" & confu_mat_0.5$obs_class=="event"])

specificity_0.5 <- (confu_mat_0.5$n[confu_mat_0.5$pred_class=="non_event" & confu_mat_0.5$obs_class=="non_event"]) / (confu_mat_0.5$n[confu_mat_0.5$pred_class=="non_event" & confu_mat_0.5$obs_class=="non_event"] + confu_mat_0.5$n[confu_mat_0.5$pred_class=="event" & confu_mat_0.5$obs_class=="non_event"])
...
```{r, solution_03a}
confu_mat_0.7 <- model_class_0.7 %>% count(pred_class, obs_class)
```

```{r}
accuracy_0.7 <- (confu_mat_0.7$n[confu_mat_0.7$pred_class=="event" & confu_mat_0.7$obs_class=="event"] + confu_mat_0.7$n[confu_mat_0.7$pred_class=="non_event" & confu_mat_0.7$obs_class=="non_event"]) / (confu_mat_0.7$n[confu_mat_0.7$pred_class=="event" & confu_mat_0.7$obs_class=="event"] + 
  confu_mat_0.7$n[confu_mat_0.7$pred_class=="event" & confu_mat_0.7$obs_class=="non_event"] + 
  confu_mat_0.7$n[confu_mat_0.7$pred_class=="non_event" & confu_mat_0.7$obs_class=="event"] + 
  confu_mat_0.7$n[confu_mat_0.7$pred_class=="non_event" & confu_mat_0.7$obs_class=="non_event"])
...
```{r, solution_03b}
sensitivity_0.7 <- (confu_mat_0.7$n[confu_mat_0.7$pred_class=="event" & confu_mat_0.7$obs_class=="event"]) / (confu_mat_0.7$n[confu_mat_0.7$pred_class=="event" & confu_mat_0.7$obs_class=="event"] + confu_mat_0.7$n[confu_mat_0.7$pred_class=="non_event" & confu_mat_0.7$obs_class=="event"])

specificity_0.7 <- (confu_mat_0.7$n[confu_mat_0.7$pred_class=="non_event" & confu_mat_0.7$obs_class=="non_event"]) / (confu_mat_0.7$n[confu_mat_0.7$pred_class=="non_event" & confu_mat_0.7$obs_class=="non_event"] + confu_mat_0.7$n[confu_mat_0.7$pred_class=="event" & confu_mat_0.7$obs_class=="non_event"])
...
```{r, solution_03c}
confu_mat_0.3 <- model_class_0.3 %>% count(pred_class, obs_class)
```

```{r}
accuracy_0.3 <- (confu_mat_0.3$n[confu_mat_0.3$pred_class=="event" & confu_mat_0.3$obs_class=="event"] + confu_mat_0.3$n[confu_mat_0.3$pred_class=="non_event" & confu_mat_0.3$obs_class=="non_event"]) / (confu_mat_0.3$n[confu_mat_0.3$pred_class=="event" & confu_mat_0.3$obs_class=="event"] + confu_mat_0.3$n[confu_mat_0.3$pred_class=="event" & confu_mat_0.3$obs_class=="non_event"] + confu_mat_0.3$n[confu_mat_0.3$pred_class=="non_event" & confu_mat_0.3$obs_class=="event"] + confu_mat_0.3$n[confu_mat_0.3$pred_class=="non_event" & confu_mat_0.3$obs_class=="non_event"])
...
```{r, solution_03d}
sensitivity_0.3 <- (confu_mat_0.3$n[confu_mat_0.3$pred_class=="event" & confu_mat_0.3$obs_class=="event"]) / (confu_mat_0.3$n[confu_mat_0.3$pred_class=="event" & confu_mat_0.3$obs_class=="event"] + confu_mat_0.3$n[confu_mat_0.3$pred_class=="non_event" & confu_mat_0.3$obs_class=="event"])

specificity_0.3 <- (confu_mat_0.3$n[confu_mat_0.3$pred_class=="non_event" & confu_mat_0.3$obs_class=="non_event"]) / (confu_mat_0.3$n[confu_mat_0.3$pred_class=="non_event" & confu_mat_0.3$obs_class=="non_event"] + confu_mat_0.3$n[confu_mat_0.3$pred_class=="event" & confu_mat_0.3$obs_class=="non_event"])
...
```{r, solution_03e}
df_3e <- data.frame(sensi = c(sensitivity_0.3, sensitivity_0.5, sensitivity_0.7), fpr = c(fpr_0.3, fpr_0.5, fpr_0.7))
```

```{r}
df_3e %>%
  ggplot(mapping = aes(x = fpr, y = sensi)) + 
  geom_point(size = 3) + 
  geom_line() +
...
Yes the levels of the binary outcome `y` is balanced.  
...
According to the summary statistics and the dotplot `model-3` is the best.  
...
```
The false-positive and false-negtive percentages increases as the model performance decrease (accuracy decreases).  
...
roc_results %>% dotplot()
```
`Model-2` has the highest `TPR`.  
`Model-7` and `Model-3` has the lowest `FPR`.  
...
What do you think?  
The sensitivity is near `1` when threshold is very low because we are setting the model to pick up the value as "event" for almost every observation. 
And as all the values will be picked up as "event" and sensitivity is the ratio of the "event" predicted it will be near to `1`.
...
Yes the result is consistent with that calculated in previous question. 
`Model-8` is closest to a "completely ineffective model" as we can see that it `swaps` errors as the threshold changes. Dropping the threshold provides no benefit in TPR at a fixed FPR.  
...
What do you think?  
As both the `event` column and `non-event` columns gives us the probability the observation will be an event or not the addition of probability of `event` and probability of `non-event` for an observation is `1`.  
...
There are `10` unique values of the `pred_bin`.
...
Yes the bin (0.9, 1] consists only of the `events`.  
No there are no bins with only `non-events`.  
...
Yes the empirical proportions of the event are consistent with predicted probability of the model-1.  
...
Add as many code chunks as you feel are necessary.  
```{r}
a = levels(test_df_1_b$pred_bin %>% unique())
a = as.factor(a)
a
```

```{r}
x = test_df_1_b[test_df_1_b$y == "event",]
x1 = x %>% count(pred_bin)
x2 = test_df_1_b %>% count(pred_bin)
x3 = x1/x2
x3 = x3$n
x3
```
```{r}
bin_mid <- seq(0 + 0.1/2, 1 - 0.1/2, 0.1)
bin_mid
```


```{r}
my_calcurve_1 <- data.frame(pred_bin = a, prop_event = x3,  mid_bin = bin_mid)
my_calcurve_1
```
...
my_calcurve_1 %>% 
  ...
  coord_equal(xlim = c(0,1), ylim = c(0,1))
```
Yes the calibration curve is consistent with the stacked filled barchart created in the previous question.  
...
```{r}
y = test_df_3_b[test_df_3_b$y == "event",]
y1 = y %>% count(pred_bin)
y2 = test_df_3_b %>% count(pred_bin)
y3 = y1/y2
y3 = y3$n
y3
```
...
```{r}
my_calcurve_3 <- data.frame(pred_bin = a, prop_event = y3,  mid_bin = bin_mid)
```
...
Add as many code chunks as you feel are necessary.  
```{r}
z = test_df_8_b[test_df_8_b$y == "event",]
z1 = z %>% count(pred_bin, .drop=FALSE)
z2 = test_df_8_b %>% count(pred_bin, .drop=FALSE)
z3 = z1/z2
z3 = z3$n
z3
```
...
```{r}
my_calcurve_8 <- data.frame(pred_bin = a, prop_event = z3,  mid_bin = bin_mid)
...
What do you think?  
Out of the three calibration curves we can say that `model-3` is most well calibrated.  
From the calibration we are able to identify `model-8` is the worst but there is mot much difference in `model-1` and `model-3`.  
The most obvious aspect of performance is that the values of `prop-event` should be closer to the liner straight line from `(0,0)`.  
...
**Yes the created calibration curve is consistent with your manually created curve from the previous problem.**
