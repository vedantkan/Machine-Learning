...
The relationship looks to be non-linear. As for the small values of `|x|` the value of response `y` is small.  
Where as the value of `|x|` increases the value of repsonse `y` increases.
...
```{r}
my_df <- data.frame(x = x_order, y = y_rmse)
...
```{r, solution_01d}
### add as many code chunks as necessary
rsq_1 <- modelr::rsquare(fit_lm_1, df_low_high)
rsq_2 <- modelr::rsquare(fit_lm_2, df_low_high)
rsq_3 <- modelr::rsquare(fit_lm_3, df_low_high)
rsq_4 <- modelr::rsquare(fit_lm_4, df_low_high)
rsq_5 <- modelr::rsquare(fit_lm_5, df_low_high)
rsq_6 <- modelr::rsquare(fit_lm_6, df_low_high)
rsq_7 <- modelr::rsquare(fit_lm_7, df_low_high)
rsq_8 <- modelr::rsquare(fit_lm_8, df_low_high)
rsq_9 <- modelr::rsquare(fit_lm_9, df_low_high)
```

```{r}
x_order <- seq(1,9)
y_rsq <- c(rsq_1, rsq_2, rsq_3, rsq_4, rsq_5, rsq_6, rsq_7, rsq_8, rsq_9)
```


```{r}
my_df_rsq <- data.frame(x = x_order, y = y_rsq)
...
According to the training set performance model 9 with `9th degree` polynomial performs best.  
The optimal value of RMSE should be least and R-squared be maximum.  
Yes according to both matrics model-9 is the best performing model.  
...
What do you think?  
The coefficient estimate of model-6 and model-9 are almost similar with difference in standard error.  
No the level of uncertainty increases with the degree of polynomial.  
...
What do you think?  
The model with polynomial degree 9 is very flexible.
...
set.seed(202214)
id_train <- sample(1:nrow(df_low_high), round(nrow(df_low_high) * 0.8))
...
df_low_high %>% 
  ggplot(mapping = aes(x = x)) + 
  geom_point(mapping = aes(y = y), data = df_train, shape = 0, color = "red", size = 3) + 
  geom_point(mapping = aes(y =y), data = df_holdout, shape = 1, color = "blue", size = 3)
```
...
```{r}
x_hold_order <- seq(1,9)
y_hold_rmse <- c(rmse_hold_1, rmse_hold_2, rmse_hold_3, rmse_hold_4, rmse_hold_5, rmse_hold_6, rmse_hold_7, rmse_hold_8, rmse_hold_9)
```

```{r}
my_hold_df <- data.frame(x = x_hold_order, y = y_hold_rmse)
...
What do you think?  
According to the hold-out evaluation model-2 is the best.  
No it is not the same model as identified from the training set performance.  
The hold-out set rank the model-9 (best according to the training set) as the worst performing model.  
...
What do you think?  
There are some differences in the value of the coefficients where in the case if `x^4` we can see the co-ef of `fit_lm` is negative wheres for `fit_train` it is positive.  
But the level of uncertainties is almost similar in both cases.  
...
What do you think?  
From the figure we can observe that the the coefficient of `x` for `fot_train` model is not statistically significant whereas for `fit_lm` it is.  
...
The model will be trained and tested 5 times with this scheme.  
...
RMSE is the most common performance metric.  
MAC is similar to RMSE but instead of squaring it takes the absolute difference between actual and predicted value.       
...
```{r, solution_04e_b}
lowhigh_results %>% 
  dotplot(metric = my_metric)
```

Which model is the best?  
According to the dotplot and also the summary statistics model-3 with 3rd degree of polynomial is the best.  
...
What do you think?
No the coefficients are samller than the ones predicted before and the level is uncertainity is also very less compared to previous ones.  
...
How does it compare?  
In this figure we can visualize directly from the figure that this is a parabola unlike the first picture where the data points were scattered far from each other to tell.  
...
Which model is the best?  
According to the summary statistics and dotplot model-6 with 6th degree polynomial is the best model.  
...
What do you think?  
No the resampling scheme identified different model as the best model for two noise.  
Yes the models identified poor in high noise case remains poor.  
...
How does it compare?  
There are more data points compared to first figure so the points doesn't look much scattered as in the first figure even though this also has high noise.  
...
Which model is the best?  
According to summary statistics and dotplot model-5 with 5th degree polynomial is the best.  
...
What do you think?  
No the resampling scheme does not identify same models as the best model for two sample size cases.  
Only model-9 remains poor in both cases whereas model-7 and model-8 seems to improve.  
...
What do you think?  
The larger sample size allows more precise estimate of the features.  
More noise will tend to decrease the accuracy of the classifier and increase the complexity of the model.  
