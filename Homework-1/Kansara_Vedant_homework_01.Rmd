...
There are different box plot for each unique value of color based on price.   
The box represents the range of price for each color category.  
The middle line inside the box represents the median.   
The lines represent the max and the min value of price of diamond for each color.   
Confused about the darkened line at the top but guessing it might just be several outliers clustered together looking like a line.
...
The boxplot in Problem 2b just gives the distribution of price of diamonds based on color.    
In the above figure of problem 2f gives us the idea of how the price varies based on `color`, `carat`, and conditioned on `cut="Ideal"` and `clarity="IF"`    
We see that prices of the diamonds which have value of carat greater than the median(carat) are higher. 
...
mpg %>%
  ggplot(mapping = aes(x = displ, y = hwy)) +
  geom_point() + 
  geom_smooth(formula = y~x, method = lm, mapping = aes(color = drv))
```
...
mpg %>%
  ggplot(mapping = aes(x = displ, y = hwy)) +
  geom_point() + 
  geom_smooth(formula = y~x, method = lm, mapping = aes(group = drv))
```
...
mod1 %>% 
  coefplot::coefplot()
```
Since the `displ` is always negetive irrespective of the value it is a significant variable.
